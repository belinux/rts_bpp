...
MODULE_DESCRIPTION("Realtek BarossaPlusPlus card reader driver");
MODULE_LICENSE("GPL");
...
{
	return "SCSI emulation for Realtek BarossaPlusPlus card reader";
}
...
static int rtsx_control_thread(void * __dev)
{
	...
	for(;;) {
		...
		if (rtsx_chk_stat(chip, RTSX_STAT_DISCONNECT)) {
			printk(KERN_INFO "-- rtsbpp-control exiting\n");
			mutex_unlock(&dev->dev_mutex);
			break;
		}
...
static void rtsx_init_options(struct rtsx_chip *chip)
{
	...
	chip->sd_sdr104_perf_adjust_clk = 98;
	...
	chip->sdr104_drive_sel_1v8 = 0x04;
	...
	chip->card_reset_debounce = 1;
	chip->card_release_debounce = 1;
	
	chip->auto_delink_in_L1 = 0;
	chip->blink_led = 1;
	chip->msi_en = msi_en;
	chip->pmos_self_open_mode = 0;

	chip->rtl8411_ldo_tuned18 = ASIC_1V8;
	chip->rtl8402_ldo_tuned18 = ASIC_2V0;
...
static int rtsx_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
	...
	th = kthread_create(rtsx_scan_thread, dev, "rtsbpp-scan");
	if (IS_ERR(th)) {
	...
	th = kthread_run(rtsx_polling_thread, dev, "rtsbpp-polling");
	if (IS_ERR(th)) {
...
static struct pci_device_id rts_bpp_ids[] = {
	{ 0x10EC, 0x5289, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_OTHERS << 16, 0xFF0000 },
	{ 0x10EC, 0x5287, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_OTHERS << 16, 0xFF0000 },
	{ 0x10EC, 0x5286, PCI_ANY_ID, PCI_ANY_ID, PCI_CLASS_OTHERS << 16, 0xFF0000 },
	{ 0, },
};

MODULE_DEVICE_TABLE(pci, rts_bpp_ids);
...
static int __init rts_bpp_init(void)
{
...
static void __exit rts_bpp_exit(void)
{
...
module_init(rts_bpp_init)
module_exit(rts_bpp_exit)
